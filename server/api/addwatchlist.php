...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_SERVER["REQUEST_URI"] == "/api/addwatchlist.php") {
    ...
    if ($user_id !== false) {
        ...
        $success = addWatchlist($show_name, $user_id);
        if(!$success){
            http_response_code(500);
            echo json_encode(array("error" => "An error occurred while adding watchlist"));
            exit;
        }
        else{
            echo json_encode(array("message" => "watchlist added"));
        }
