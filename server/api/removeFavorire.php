...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_SERVER["REQUEST_URI"] == "/api/removeFavorire.php") {
    ...
    if($user_id !== false){
        $success = removeFavorite($show_name, $user_id);
        if(!$success){
            http_response_code(500);
            echo json_encode(array("error" => "An error occurred while removing favorite"));
            exit;
        }
        else{
